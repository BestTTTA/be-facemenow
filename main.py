from fastapi import FastAPI, UploadFile, File, HTTPException, Form, Depends, Query, Request
...
async def create_duser(duser: DUserCreate):
    ...
    execute_query(query, (
        duser_id, duser.display_name, duser.display_details, duser.profile_url, 
        duser.consent, current_time, current_time
    ))
    
    return {
        "message": "Donor user created successfully",
        "duser_id": duser_id,
...
async def create_checkout_session(payment_data: StripePaymentIntent):
    try:
        session = stripe.checkout.Session.create(
            ...
            mode="payment",
            payment_intent_data={  # ✅ เพิ่มตรงนี้
                "metadata": {
                    "event_id": payment_data.event_id,
                    "duser_id": payment_data.duser_id,
                    "original_amount": str(payment_data.amount)
    ...
    try:
        ...
        query = """
            INSERT INTO leaderboard (id, event_id, duser_id, donate_price, stripe_payment_intent_id,
                                     payment_status, create_at, update_at)
            VALUES (%s, %s, %s, %s, %s, 'completed', %s, %s)
        """
        execute_query(query, (
            donation_id, event_id, duser_id, amount,
            stripe_payment_intent_id, current_time, current_time
        ))
...
async def handle_successful_payment(payment_intent):
    ...
    try:
        ...
        execute_query(donation_query, (
            ...
            'completed', current_time, current_time
        ))
